...
<?php if(isset($_REQUEST['f'])==1){?><p align="center" style="color:#FF0000; padding-bottom:8px;">Transfer already exists!</p><?php }?>
<?php if(isset($_REQUEST['m'])==1){?><p align="center" style="color:#00CC00; padding-bottom:8px;">Transfer Update Successfully</p><?php }?>
<?php 
$sql="SELECT * FROM `da_settings_transfer` WHERE `id`='".trim($_REQUEST['id'])."'";
$res=query($conn,$sql);
$num=numrows($res);
if($num>0)
{
...
<input type="text" id="chargeper" name="chargeper" class="form-control  border-primary" placeholder="Charge(%)" value="<?=$fetch['chargeper']?>" required>         
</div>
...
<td style="text-align:center;padding:3px;"><?=$fetch['chargeper']?> %</td>
