...
<?php 
$sql="SELECT * FROM `da_settings_withdrawal` WHERE `id`='".$_REQUEST['id']."'";
$res=query($conn,$sql);
$num=numrows($res);
if($num>0)
{
...
<input type="text" id="maximum" name="maximum" class="form-control  border-primary" placeholder="Maximum" value="<?=$fetch['maximum']?>" required>         
</div>
...
<div class="form-group">
<label for="userinput5">TDS(%)<b style="color:#FF0000;">*</b></label>
<input type="text" id="tds" name="tds" class="form-control  border-primary" placeholder="TDS"value="<?=$fetch['tds']?>" required>                            
</div>   


<div class="form-group">
<label for="userinput5">Admin(%)<b style="color:#FF0000;">*</b></label>
<input type="text" id="admin" name="admin" class="form-control  border-primary" placeholder="Admin"value="<?=$fetch['admin']?>" required>                            
</div>   


<div class="form-group">
<label for="userinput5">No Of Direct<b style="color:#FF0000;">*</b></label>
...
<div class="form-group">
<label for="userinput5">IMPS<b style="color:#FF0000;">*</b></label>
<input type="text" id="imps" name="imps" class="form-control  border-primary" placeholder="IMPS"value="<?=$fetch['imps']?>" required>                            
</div>
...
<td style="text-align:center;padding:3px;"><?=$fetch['minimum']?></td>
<td style="text-align:center;padding:3px;"><?=$fetch['maximum']?></td>
<td style="text-align:center;padding:3px;"><?=$fetch['tds']?> %</td>
<td style="text-align:center;padding:3px;"><?=$fetch['admin']?> %</td>
<td style="text-align:center;padding:3px;"><?=$fetch['nodirect']?></td>
<td style="text-align:center;padding:3px;"><?=$fetch['imps']?></td>
<td align="center" style="padding:3px;">
<a href="settings-withdrawal.php?case=edit&id=<?=$fetch['id']?>" onClick="return confirm('Are you sure want to Edit?');"><img src="images/edit.png" /></a>&nbsp;
</td>
</tr>
<?php $i++;}}else{?>
