...
<h4 class="card-title">View Kyc Approved Statement</h4>
...
$tname='da_member_kyc';
$lim=100;
$tpage='kyc-details-approved.php';

if($_REQUEST['act']=='search')
{
$where="WHERE `userid` LIKE '".trim($_POST['search'])."' AND `pancardstatus`='A' AND`aadharfrontstatus`='A' AND`aadharbackstatus`='A' AND`status`='A' ORDER BY `id` DESC";
}else{
$where="WHERE `pancardstatus`='A' AND`aadharfrontstatus`='A' AND`aadharbackstatus`='A' AND`status`='A' ORDER BY `id` DESC";
}
