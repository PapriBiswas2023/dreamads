...
$arr[0][6]="Tron_Wallet";
$arr[0][7]="Date";
$arr[0][8]="Approved date";
$arr[0][9]="Status";
$arr[0][10]="Pay status";
$arr[0][11]="Renewal status";

if($_REQUEST['formdate'] && $_REQUEST['todate'])
{ 

$sqlm="SELECT * FROM `da_member` WHERE `date` BETWEEN '".$_REQUEST['formdate']."'  AND '".$_REQUEST['todate']."' AND `paystatus`='I' ORDER BY `id` DESC";
}else{
$sqlm="SELECT * FROM `da_member` WHERE `paystatus`='I' ORDER BY `id` DESC";
}
