...
{
$sql2="SELECT * FROM `da_settings_rank-capping` WHERE `level`='".trim($_REQUEST['level'])."' AND `id`!='".trim($_REQUEST['id'])."'";
$res2=query($conn,$sql2);
$num2=numrows($res2);
if($num2>0)
{
...
$sql2="UPDATE `da_settings_reward` SET `level`='".trim($_POST['level'])."',`rank`='".mysqli_real_escape_string($conn,$_POST['rank'])."',`team`='".mysqli_real_escape_string($conn,$_POST['team'])."',`capping`='".trim($_POST['capping'])."' WHERE `id`='".mysqli_real_escape_string($conn,$_REQUEST['id'])."'";
$res2=query($conn,$sql2);
}
