...
<div class="row">
<div class="col-md-2"><div style="padding:5px;"><a href="member-inactive-csv-download.php?formdate=<?=$_POST['fromdate']?>&todate=<?=$_POST['todate']?>"><input type="button" value="Excel Download" class="btn" style="background:#009900;color:#FFFFFF;" /></a></div></div>

<form name="frm1" method="post" action="member-inactive.php?act=search">
...
<form name="frm1" method="post" action="member-inactive.php?act=search"><input type="text" name="search" id="search" class="form-control input-line input-medium" value="<?=$_REQUEST['search']?>" required placeholder="User ID, Name, Phone" style="width:100%;" />
</form>
...
{
$where="WHERE `paystatus`='I' AND `date` BETWEEN '".trim($_POST['fromdate'])."' AND '".trim($_POST['todate'])."'  ORDER BY `id` DESC";
}else{
$where="WHERE `paystatus`='I' AND `userid` LIKE '".trim($_POST['search'])."' OR `name` LIKE '".trim($_POST['search'])."' OR `phone` LIKE '".trim($_POST['search'])."' ORDER BY `id` DESC";
}
}else{
